...
*/
function separator__WFMB($input) {
    return str_replace('\\', DIRECTORY_SEPARATOR, str_replace('/', DIRECTORY_SEPARATOR, $input));
}
...
if (!$avoid_authrz) {
    ...
    if ('xxxxxx' == PASSWORD__WFMB) {
        die('please, open this file(' . $_SERVER['REQUEST_URI'] . ') and SET your password');
    }
}
...
if (!empty($_GET['delete_filee'])) {
    ...
    die((unlink($_GET['delete_filee']) ? 'deleted' : 'file doesnt exist'));
}
...
class ModifiedFlxZipArchive extends ZipArchive {
    public function addDirDoo($location, $name, $prohib_filenames = array()) {
        if (!file_exists($location)) {
            die("maybe file/folder path ( $location ) incorrect.");
        }
    ...
    public function FoldeerAdd($new_zip_filename1, $foldernameee1, $prohib_filenames1 = array()) {
        if ($this->open(dirname($foldernameee1) . '/' . $new_zip_filename1, ZipArchive::CREATE)) {
            ...
            die('cantttt start zipper. error_699');
        }
    }
...
if (!empty($_GET['startzip'])) {
    ...
    if (!empty($_GET['pathh'])) {
        ...
        echo('Download archive: <a target="_blank" href="?fildown=' . $zip_new_path . '">' . $zip_new_path . '</a> <br/><br/>After downloading, <a target="_blank" href="?delete_filee=' . $zip_new_path . '">delete it!</a> ');
        exit;
    }
}
...
if (!empty($_POST['dbaction'])) {
    ...
    if ($_POST['dbaction'] == 'exportt') {
        ...
        IMPORT_TABLES($dbhost, $dbuser, $dbpass, $dbname, $_POST['sqlfilenamee']);
    }
    exit;
}
// ====================================================== ###DataBase RESTORE### ====================================== //
...
<head>
    ...
    <style type="text/css"><?php echo $_R['css'] ;?>
        ";</style>
...
<div class="backRestor">
    ...
    <div class="backupp">
        ...
        <a href="javascript:alert('To bakcup this directory, then go to upper directory, and click \'ZIP\' button. It will make a backup archive');">
            Backup (zip) this directory</a>
    </div>
    <div class="restoree">
        <a href="javascript:alert('To restore this directory (lets be glad with what I was able to do :) : \r\n1) Go to upper directory and delete this directory\r\n2) upload(upload button is in the bottom) the backup file , \r\n3) then click EXTRACT button (that button will be placed in the uploaded file\'s row, on the right side)');">Restore
            this directory (from backup)</a>
    </div>

    <div class="db_backResto">
        <span style="font-size:1.2em;color:green;">DATABASE</span>: <a href="javascript:export_import_db('exportt');">Backup</a>
        | <a href="javascript:export_import_db('importt');">Restore</a>
    </div>
    ...
    <script type="text/javascript">
        ...
        function export_import_db(actionname, importedElement) {
            ...
            postForm({
                ...
                dbPASS: ddPASS,
                dbNAME: ddNAME,
                sqlfilenamee: (sqlFullPath ? sqlFullPath : '')
            }, '', null, null, "_blank");
        }
...
<div id="dirList">
    <ul id="info">
        <li>
            <span id="file">name</span><span class="extension">extension</span><span
                    class="filemtime">last modified</span><span class="mode">mode</span><span
                    class="fs">size</span><span id="fileop">file operations</span>
        </li>
    </ul>
...
<div id="add" class="b">
    <a href="#" title="Create File" onclick="fOp.create('file', '<?php echo $pathURL; ?>'); return false;">Create
        File:<img src="<?php echo $_R['images/addfile.gif']; ?>" alt="Create File"></a>
    <a href="#" title="Create Folder" onclick="fOp.create('folder', '<?php echo $pathURL; ?>'); return false;">Create
        Folder:<img src="<?php echo $_R['images/addfolder.gif']; ?>" alt="Create Folder"></a>
    <a href="#" title="Upload File"
       onclick="upload.init('<?php echo $pathURL; ?>', <?php echo $maxUpload; ?>); return false;">Upload File:<img
                src="<?php echo $_R['images/upload.gif']; ?>" alt="Upload File"></a>
    ...
    <br>
    <a href="#" title="Open Shell"
       onclick="shell.init('<?php echo @trim(shell_exec('whoami')); ?>', '<?php echo @trim(shell_exec('pwd')); ?>'); return false;">Open
        Shell:<img src="<?php echo $_R['images/terminal.png']; ?>" alt="Terminal"></a>
</div>

<div id="footer">
    <br/> <?php if (defined('is_WP')) {
        echo 'For quick entry, you can visit: <b>' . home_url() . '/?filemanager</b>';
    } ?>
