...
###    * Using 'replace_src'=>true, it fixes domain-relative urls:
###         (i.e.:   src="./file.jpg"  ----->  src="http://example.com/file.jpg" )
###    * Using 'schemeless'=>true, it converts urls in schemeless:
###         (i.e.:   src="http://exampl..  ----->  src="//exampl... )\
...
function get_remote_data($url, $post_paramtrs=false, $extra=array('schemeless'=>true, 'replace_src'=>true, 'return_array'=>false)){
    ...
    if(function_exists('url_corrections_for_content_HELPER')){
        $data= url_corrections_for_content_HELPER($data, $status['url'],
            array('schemeless'=>!empty($extra['schemeless']), 'replace_src'=>!empty($extra['replace_src']), 'rawgit_replace'=>!empty($extra['rawgit_replace'])
            )
        );
    }
                ...
                foreach($GLOBALS['rdgr']['ext_array'] as $key=>$value){
                    if(in_array($tagname,$value)){
                        ...
                        if(!empty($n[1])){
                            ...
                            $content_A = preg_replace_callback('/( '.$key.'='.$GLOBALS['rdgr']['aphostrope_type'].')(.*?)('.$GLOBALS['rdgr']['aphostrope_type'].')/i',
                                function($matches_B){
                                    ...
                                    if(!empty($GLOBALS['rdgr']['replace_src'])){
                                        ...
                                        if(substr($full_link, 0,2) != '//'){
                                            ...
                                            foreach($GLOBALS['rdgr']['all_protocols'] as $each_protocol){
                                                //if protocol found - don't continue
