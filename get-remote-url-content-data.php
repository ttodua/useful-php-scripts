...
function get_remote_data($url, $post_paramtrs=false,               $return_full_array=false)	{
	...
	if($post_paramtrs){ curl_setopt($c, CURLOPT_POST,TRUE);	if(is_array($post_paramtrs){ curl_setopt_array ($c ,  $post_paramtrs);) }  else { curl_setopt($c, CURLOPT_POSTFIELDS, "var1=bla&".$post_paramtrs );}  }		curl_setopt($c, CURLOPT_SSL_VERIFYHOST,false);                  
